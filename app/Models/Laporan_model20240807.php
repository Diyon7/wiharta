...
class Laporan_model extends Model
{
    ...
    public function Dataabsen($form)
    {
        ...
        for ($a = $form['tagihanawal']; $a <= $form['tagihanakhir']; $a->modify('+ 1 day')) {
            ...
            $serialinput = "'FIO616202022321246', 'FIO616202023120302', 'FIO616202022411175', 'FIO616203019510399'";
            $serialoutput = "'FIO616202023120332', 'FIO616202022322194', 'FIO616202023120165', 'FIO616202022322311','616202024171373','616202024171066'";
            ...
            if ($izincekinmin != null) {
                ...
                $cekinmin = $this->select("MIN(DATE_FORMAT(att_log.`scan_date`, '%y-%m-%d %H:%i:%s')) AS cekin")
                    ...
                    ->where("(att_log.`sn` in ($serialinput) OR (att_log.`sn`='FIO616202022321281' AND att_log.`inoutmode` in ($qinput)))")
                    ->get()->getResultArray();
                ...
                $cekoutmax2 = $this->select("MAX(DATE_FORMAT(att_log.`scan_date`, '%y-%m-%d %H:%i:%s')) AS cout, att_log.`inoutmode` AS inoutm")
                    ...
                    ->where('pegawai.`pegawai_nip`', $form['idkar'])
                    ->where("scan_date BETWEEN '$timeawal 00:00:00' AND '$timeawal 23:59:59'")
                    ->where("(att_log.`sn` in ($serialoutput) OR (att_log.`sn`='FIO616202022321281' AND att_log.`inoutmode` in ($qoutput)))")
                    ->get()->getResultArray();
                ...
                $siftl = $this->db->query("SELECT COUNT(s1) AS s1, COUNT(s2) AS s2, COUNT(s3) AS s3 FROM pegawai
                    LEFT JOIN (SELECT pin AS s1 FROM att_log WHERE ((att_log.`sn` in ($serialinput) OR (att_log.`sn`='FIO616202022321281' AND att_log.`inoutmode` in ($qinput))) AND att_log.`scan_date` BETWEEN '$timeawal 05:00:00' AND '$timeawal 12:30:00') GROUP BY pin) AS ls1 ON pegawai.`pegawai_pin`=ls1.s1
                    LEFT JOIN (SELECT pin AS s2 FROM att_log WHERE ((att_log.`sn` in ($serialinput) OR (att_log.`sn`='FIO616202022321281' AND att_log.`inoutmode` in ($qinput))) AND att_log.`scan_date` BETWEEN '$timeawal 13:30:00' AND '$timeawal 20:00:00') GROUP BY pin) AS ls2 ON pegawai.`pegawai_pin`=ls2.s2
                    LEFT JOIN (SELECT pin AS s3 FROM att_log WHERE ((att_log.`sn` in ($serialinput) OR (att_log.`sn`='FIO616202022321281' AND att_log.`inoutmode` in ($qinput))) AND att_log.`scan_date` BETWEEN '$timeawal 21:30:00' AND '$timeshift') GROUP BY pin) AS ls3 ON pegawai.`pegawai_pin`=ls3.s3
