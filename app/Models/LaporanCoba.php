...
class Laporan_model extends Model
{
    ...
    public function Alllaporan($data)
    {
        ...
        WHERE pembagian3.`pembagian3_nama` LIKE '" . $data['vendor'] . "' AND pegawai.`tgl_mulai_kerja` <= '" . $data['tgl2'] . "' AND pegawai.`pegawai_nip` REGEXP '^[0-9]+$' AND pegawai.`golongan`!='1' AND (tgl_resign >= '" . $data['tgl'] . "' OR tgl_resign IS NULL)")
            ->getResultArray();
