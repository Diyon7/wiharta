...
class RekapAll_model extends Model
{
    ...
    public function DKPMasuk($data)
    {
        ...
        $timeshift = date('Y-m-d H:i:s', strtotime(' +1 days +1 hours', strtotime($tanggal . ' 00:10:00')));
        $day = date('Y-m-d H:i:s', strtotime(' +1 days', strtotime($tanggal . ' 08:30:00')));
        return $this->db->query("SELECT pembagian4.`pembagian4_nama`, COUNT(pembagian4.`pembagian4_nama`) AS DKP, mk, mks1, mks2, mks3, sh0, dlk FROM pegawai
        JOIN pembagian4 ON pembagian4.`pembagian4_id`=pegawai.`pembagian4_id`
        LEFT JOIN pembagian2 ON pegawai.`pembagian2_id`=pembagian2.`pembagian2_id`
        ...
        WHERE (pegawai.`tgl_resign`='0000-00-00' OR pegawai.`tgl_resign`>='" . $data['tanggal'] . "' OR pegawai.`tgl_resign` IS NULL) AND pegawai.`tgl_mulai_kerja`<='" . $data['tanggal'] . "' AND tb_jadwal.`tgl`='" . $data['tanggal'] . "' AND pembagian2.`pembagian2_nama` LIKE '" . $data['divisi'] . "' AND " . $data['status'] . " GROUP BY pembagian4.`pembagian4_nama`) AS s1 ON s1.pembagian4_id=pembagian4.`pembagian4_id`
        LEFT JOIN(SELECT pembagian4.`pembagian4_nama`, pegawai.`pembagian4_id`, COUNT(pembagian4.`pembagian4_nama`) AS mks1 FROM pegawai
        LEFT JOIN tb_jadwal ON tb_jadwal.`grupass`=pegawai.`grup_jam_kerja`
        LEFT JOIN pembagian2 ON pegawai.`pembagian2_id`=pembagian2.`pembagian2_id`
        JOIN pembagian4 ON pembagian4.`pembagian4_id`=pegawai.`pembagian4_id`
        JOIN (SELECT pin FROM att_log WHERE (att_log.`inoutmode` in ('1', '3', '5', '7', '9') AND att_log.`scan_date` BETWEEN '" . $data['tanggal'] . " 05:00:00' AND '" . $data['tanggal'] . " 12:30:00') GROUP BY pin) AS pin ON pegawai.`pegawai_pin`=pin.`pin`
        WHERE (pegawai.`tgl_resign`='0000-00-00' OR pegawai.`tgl_resign`>='" . $data['tanggal'] . "' OR pegawai.`tgl_resign` IS NULL) AND pegawai.`tgl_mulai_kerja`<='" . $data['tanggal'] . "' AND tb_jadwal.`tgl`='" . $data['tanggal'] . "' AND pembagian2.`pembagian2_nama` LIKE '" . $data['divisi'] . "' AND " . $data['status'] . " GROUP BY pembagian4.`pembagian4_nama`) AS ss1 ON ss1.pembagian4_id=pembagian4.`pembagian4_id`
        LEFT JOIN(SELECT pembagian4.`pembagian4_nama`, pegawai.`pembagian4_id`, COUNT(pembagian4.`pembagian4_nama`) AS mks2 FROM pegawai
        LEFT JOIN tb_jadwal ON tb_jadwal.`grupass`=pegawai.`grup_jam_kerja`
        LEFT JOIN pembagian2 ON pegawai.`pembagian2_id`=pembagian2.`pembagian2_id`
        JOIN pembagian4 ON pembagian4.`pembagian4_id`=pegawai.`pembagian4_id`
        JOIN (SELECT pin FROM att_log WHERE (att_log.`inoutmode` in ('1', '3', '5', '7', '9') AND att_log.`scan_date` BETWEEN '" . $data['tanggal'] . " 13:30:00' AND '" . $data['tanggal'] . " 20:00:00') GROUP BY pin) AS pin ON pegawai.`pegawai_pin`=pin.`pin`
        WHERE (pegawai.`tgl_resign`='0000-00-00' OR pegawai.`tgl_resign`>='" . $data['tanggal'] . "' OR pegawai.`tgl_resign` IS NULL) AND pegawai.`tgl_mulai_kerja`<='" . $data['tanggal'] . "' AND tb_jadwal.`tgl`='" . $data['tanggal'] . "' AND pembagian2.`pembagian2_nama` LIKE '" . $data['divisi'] . "' AND " . $data['status'] . " GROUP BY pembagian4.`pembagian4_nama`) AS ss2 ON ss2.pembagian4_id=pembagian4.`pembagian4_id`
        LEFT JOIN(SELECT pembagian4.`pembagian4_nama`, pegawai.`pembagian4_id`, COUNT(pembagian4.`pembagian4_nama`) AS mks3 FROM pegawai
        LEFT JOIN tb_jadwal ON tb_jadwal.`grupass`=pegawai.`grup_jam_kerja`
        LEFT JOIN pembagian2 ON pegawai.`pembagian2_id`=pembagian2.`pembagian2_id`
        JOIN pembagian4 ON pembagian4.`pembagian4_id`=pegawai.`pembagian4_id`
        JOIN (SELECT pin FROM att_log WHERE (att_log.`inoutmode` in ('1', '3', '5', '7', '9') AND att_log.`scan_date` BETWEEN '" . $data['tanggal'] . " 21:30:00' AND '$timeshift') GROUP BY pin) AS pin ON pegawai.`pegawai_pin`=pin.`pin`
        WHERE (pegawai.`tgl_resign`='0000-00-00' OR pegawai.`tgl_resign`>='" . $data['tanggal'] . "' OR pegawai.`tgl_resign` IS NULL) AND pegawai.`tgl_mulai_kerja`<='" . $data['tanggal'] . "' AND tb_jadwal.`tgl`='" . $data['tanggal'] . "' AND pembagian2.`pembagian2_nama` LIKE '" . $data['divisi'] . "' AND " . $data['status'] . " GROUP BY pembagian4.`pembagian4_nama`) AS ss3 ON ss3.pembagian4_id=pembagian4.`pembagian4_id`
        LEFT JOIN(SELECT pembagian4.`pembagian4_nama`, pegawai.`pembagian4_id`, COUNT(pembagian4.`pembagian4_nama`) AS sh0 FROM pegawai
        LEFT JOIN tb_jadwal ON tb_jadwal.`grupass`=pegawai.`grup_jam_kerja`
        LEFT JOIN pembagian2 ON pegawai.`pembagian2_id`=pembagian2.`pembagian2_id`
        JOIN pembagian4 ON pembagian4.`pembagian4_id`=pegawai.`pembagian4_id`
    ...
    public function Detailunit($data)
    {
		...
		WHERE DATE(att_log.`scan_date`)='" . $data['tanggal'] . "' AND att_log.`inoutmode` in ('1','3','5','7','9') GROUP BY pegawai.`pegawai_pin`) AS fingerm ON fingerm.idpegawai=pegawai.`pegawai_pin`
		LEFT JOIN (SELECT MAX(TIME(scan_date)) AS pulang, pegawai.`pegawai_pin` AS idpegawai FROM pegawai
		JOIN att_log ON att_log.`pin`=pegawai.`pegawai_pin`
		WHERE att_log.`scan_date` BETWEEN '" . $data['tanggal'] . " 14:00:00' AND '$day' AND att_log.`inoutmode` in ('2','4','6','8') GROUP BY pegawai.`pegawai_pin`) AS fingerp ON fingerp.idpegawai=pegawai.`pegawai_pin`
    ...
    public function Detailabsenmasuk($data)
    {
        $day = date('Y-m-d H:i:s', strtotime(' +1 days', strtotime($data['tanggal'] . ' 08:30:00')));
        return $this->db->query("SELECT pegawai.`pegawai_nama` AS nama, pegawai.`pegawai_nip` AS idkar, pembagian1.`pembagian1_nama` AS posisi, fingerm.masuk, fingerp.pulang, pembagian5.`pembagian5_nama` AS subunit, pembagian3.`pembagian3_nama` AS asal, pegawai.`tgl_mulai_kerja` AS tmt, pegawai.`grup_t` AS grup FROM pegawai
	    LEFT JOIN pembagian1 ON pembagian1.`pembagian1_id`=pegawai.`pembagian1_id`        
		...
		JOIN att_log ON att_log.`pin`=pegawai.`pegawai_pin`
		WHERE att_log.`scan_date` BETWEEN '" . $data['tanggal'] . " 14:00:00' AND '$day' AND att_log.`inoutmode` in ('2','4','6','8') GROUP BY pegawai.`pegawai_pin`) AS fingerp ON fingerp.idpegawai=pegawai.`pegawai_pin`
        WHERE (fingerm.masuk is not null or fingerp.pulang is not null) AND pegawai.`tgl_mulai_kerja` <= '" . $data['tanggal'] . "' AND (tgl_resign >= '" . $data['tanggal'] . "' OR tgl_resign IS NULL OR tgl_resign='0000-00-00')")->getResultArray();
    }

    public function Detailabsentidakmasuk($data)
    {
        $day = date('Y-m-d H:i:s', strtotime(' +1 days', strtotime($data['tanggal'] . ' 08:30:00')));
        return $this->db->query("SELECT pegawai.`pegawai_nama` AS nama, pegawai.`pegawai_nip` AS idkar, if(shift=0,'libur','') as jadwal, pembagian1.`pembagian1_nama` AS posisi, fingerm.masuk, fingerp.pulang, pembagian4.`pembagian4_nama` AS unit, pembagian5.`pembagian5_nama` AS subunit, pembagian3.`pembagian3_nama` AS asal, pegawai.`tgl_mulai_kerja` AS tmt, pegawai.`grup_t` AS grup FROM pegawai
	    LEFT JOIN pembagian1 ON pembagian1.`pembagian1_id`=pegawai.`pembagian1_id`
	    LEFT JOIN tb_jadwal ON tb_jadwal.`grupass`=pegawai.`grup_jam_kerja`   
        LEFT JOIN pembagian2 ON pembagian2.`pembagian2_id`=pegawai.`pembagian2_id`
        LEFT JOIN pembagian4 ON pembagian4.`pembagian4_id`=pegawai.`pembagian4_id`
		...
		JOIN att_log ON att_log.`pin`=pegawai.`pegawai_pin`
		WHERE att_log.`scan_date` BETWEEN '" . $data['tanggal'] . " 14:00:00' AND '$day' AND att_log.`inoutmode` in ('2','4','6','8') GROUP BY pegawai.`pegawai_pin`) AS fingerp ON fingerp.idpegawai=pegawai.`pegawai_pin`
        WHERE fingerm.masuk is null and fingerp.pulang is null AND tgl='" . $data['tanggal'] . "' AND pegawai.`tgl_mulai_kerja` <= '" . $data['tanggal'] . "' AND (tgl_resign >= '" . $data['tanggal'] . "' OR tgl_resign IS NULL OR tgl_resign='0000-00-00')")->getResultArray();
    }
}