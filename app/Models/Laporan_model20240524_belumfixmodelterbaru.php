...
class Laporan_model extends Model
{
    ...
    function __construct(RequestInterface $request = null)
    {
        ...
        $this->dt = $this->db->table("jam_kerja");
        $this->request = $request;
    }

    // public function Datacoba()
    // {
    //     return $this->db->query("SELECT jam_kerja.`jk_bcin` AS smasuk, jam_kerja.`jk_durtime` AS durasi, jam_kerja.`jk_ecout` AS skeluar FROM jam_kerja
    //      WHERE jam_kerja.jk_kode2='s1'
    //       AND jam_kerja.grup LIKE '%GK0R%'
    //        AND jam_kerja.jk_name NOT LIKE '%sabtu%'")->getRowArray();

    //     // return $this->dt->select("`jk_bcin` AS smasuk, `jk_durtime` AS durasi, `jk_ecout` AS skeluar")
    //     //     ->where("jk_kode2", 's1')
    //     //     ->like("grup", "%GK0R%")
    //     //     ->notLike("jk_name", "%sabtu%")
    //     //     ->get()
    //     //     ->getRowArray();
    // }
    ...
    {
        $k = $data['tgl'];
        return $this->db->query("SELECT pegawai.`pegawai_nama` AS nama, pegawai.`pegawai_nip` AS idkar,group_user.`jumlah_kerja` AS kjkerja, pegawai.`golongan` AS golongan, pembagian2.`pembagian2_nama` AS divisi, pembagian4.`pembagian4_nama` AS unit, pembagian5.`pembagian5_nama` AS subunit, pembagian6.`pembagian6_nama` AS kategori, pegawai.`grup_jam_kerja` AS grupt, pembagian3.`pembagian3_nama` AS asal, pegawai.`golongan` AS golongan, pegawai.`tgl_mulai_kerja` AS tmt, pegawai.`grup` AS grup FROM pegawai
        ...
        LEFT JOIN group_user ON group_user.`group_jk`=pegawai.`grup_jam_kerja`
        WHERE pembagian3.`pembagian3_nama` LIKE '" . $data['vendor'] . "' AND pegawai.`tgl_mulai_kerja` <= '" . $data['tgl2'] . "' AND (tgl_resign >= '" . $data['tgl'] . "' OR tgl_resign IS NULL OR tgl_resign='0000-00-00')")
            ->getResultArray();
    ...
    public function Dataabsen($form)
    {
        ...
        for ($a = $form['tagihanawal']; $a <= $form['tagihanakhir']; $a->modify('+ 1 day')) {
            ...
            if ($izincekinmin != null) {
                ...
                $sift = $this->db->query("SELECT pegawai_nip, shift FROM tbl_izin WHERE tbl_izin.`pegawai_nip`='" . $form['idkar'] . "' AND tbl_izin.`in` LIKE '$timeawal %' AND tbl_izin.verified='a'")->getRowArray();
                ...
                $timeshift = date('Y-m-d H:i:s', strtotime(' +1 days +1 hours', strtotime($timeawal . ' 00:10:00')));
                $siftl = $this->db->query("SELECT COUNT(s1) AS s1, COUNT(s2) AS s2, COUNT(s3) AS s3 FROM pegawai
                LEFT JOIN (SELECT pin AS s1 FROM att_log WHERE (att_log.`inoutmode` IN ('1', '3', '5', '7', '9') AND att_log.`scan_date` BETWEEN '$timeawal 05:00:00' AND '$timeawal 12:30:00') GROUP BY pin) AS ls1 ON pegawai.`pegawai_pin`=ls1.s1
                LEFT JOIN (SELECT pin AS s2 FROM att_log WHERE (att_log.`inoutmode` IN ('1', '3', '5', '7', '9') AND att_log.`scan_date` BETWEEN '$timeawal 13:30:00' AND '$timeawal 20:00:00') GROUP BY pin) AS ls2 ON pegawai.`pegawai_pin`=ls2.s2
                LEFT JOIN (SELECT pin AS s3 FROM att_log WHERE (att_log.`inoutmode` IN ('1', '3', '5', '7', '9') AND att_log.`scan_date` BETWEEN '$timeawal 21:30:00' AND '$timeshift') GROUP BY pin) AS ls3 ON pegawai.`pegawai_pin`=ls3.s3
                WHERE pegawai.`pegawai_nip`='" . $form['idkar'] . "'")->getRowArray();

                // $sift1 = $this->db->query("SELECT pegawai_nip FROM pegawai
                //             JOIN (SELECT pin FROM att_log WHERE (att_log.`inoutmode` IN ('1', '3', '5', '7', '9') AND att_log.`scan_date` BETWEEN '$timeawal 05:00:00' AND '$timeawal 12:30:00') GROUP BY pin) AS pin ON pegawai.`pegawai_pin`=pin.`pin`
                //             WHERE pegawai.`pegawai_nip`='" . $form['idkar'] . "'")->getNumRows();
                // $sift2 = $this->db->query("SELECT pegawai_nip FROM pegawai
                //             JOIN (SELECT pin FROM att_log WHERE (att_log.`inoutmode` IN ('1', '3', '5', '7', '9') AND att_log.`scan_date` BETWEEN '$timeawal 13:30:00' AND '$timeawal 20:00:00') GROUP BY pin) AS pin ON pegawai.`pegawai_pin`=pin.`pin`
                //             WHERE pegawai.`pegawai_nip`='" . $form['idkar'] . "'")->getNumRows();
                // $sift3 = $this->db->query("SELECT pegawai_nip FROM pegawai
                //             JOIN (SELECT pin FROM att_log WHERE (att_log.`inoutmode` IN ('1', '3', '5', '7', '9') AND att_log.`scan_date` BETWEEN '$timeawal 21:30:00' AND '$timeshift') GROUP BY pin) AS pin ON pegawai.`pegawai_pin`=pin.`pin`
                //             WHERE pegawai.`pegawai_nip`='" . $form['idkar'] . "'")->getNumRows();
            ...
            $datajam = null;
            if ($dayi == "Sat") {
                $datajam = $this->db->query("SELECT jam_kerja.`jk_bcin` AS smasuk, jam_kerja.`jk_durtime` AS durasi, jam_kerja.`jk_ecout` AS skeluar FROM jam_kerja
                WHERE jam_kerja.`jk_kode2`='$shift' AND jam_kerja.`grup` LIKE '%" . $form['grupt'] . "%' AND jam_kerja.`jk_name` LIKE '%sabtu%'")->getRowArray();
            } else {
                $datajam = $this->db->query("SELECT jam_kerja.`jk_bcin` AS smasuk, jam_kerja.`jk_durtime` AS durasi, jam_kerja.`jk_ecout` AS skeluar FROM jam_kerja WHERE jam_kerja.`jk_kode2`='$shift' AND jam_kerja.`grup` LIKE '%" . $form['grupt'] . "%' AND jam_kerja.`jk_name` NOT LIKE '%sabtu%'")->getRowArray();
            }
            ...
            $jadwalkerja = $this->db->query("SELECT jdw_kerja_m.`jdw_kerja_m_kode` AS kodejk, jdw_kerja_m.`jdw_kerja_m_keterangan` AS jdwtagihan, jdw_kerja_m.`jdw_kerja_m_mulai` AS tglta
            FROM jdw_kerja_m WHERE jdw_kerja_m.`jdw_kerja_m_kode`='" . $form['grupt'] . "'")->getRowArray();
            ...
            $mjkn = $this->db->query("SELECT MAX(jdw_kerja_d.`jdw_kerja_d_idx`) AS idx FROM jdw_kerja_m
                        JOIN jdw_kerja_d ON jdw_kerja_m.`jdw_kerja_m_id`=jdw_kerja_d.`jdw_kerja_m_id`
                        LEFT JOIN jam_kerja ON jam_kerja.`jk_id` = jdw_kerja_d.`jk_id`
                        WHERE jdw_kerja_m.`jdw_kerja_m_kode`='" . $jadwalkerja['kodejk'] . "'")->getRowArray();

            $dtglta = new DateTime($jadwalkerja['tglta']);
            ...
            for ($i = $dtglta; $i <= $form['tagihanakhir']; $i->modify('+ 1 day')) {
                ...
                if (date('Y-m-d', strtotime($timeawal)) === date_format($i, 'Y-m-d')) {
                    $tjk = $this->db->query("SELECT jdw_kerja_m.`jdw_kerja_m_kode` AS kodejk, jdw_kerja_d.`jdw_kerja_d_idx` AS idx, jdw_kerja_m.`jdw_kerja_m_periode` AS p, jdw_kerja_m.`jdw_kerja_m_mulai` AS tglta, jdw_kerja_d.`jdw_kerja_d_libur` AS libur, jam_kerja.`jk_bcin` AS smasuk, jam_kerja.`jk_durtime` AS durasi, jam_kerja.`jk_ecout` AS skeluar FROM jdw_kerja_m
                                    JOIN jdw_kerja_d ON jdw_kerja_m.`jdw_kerja_m_id`=jdw_kerja_d.`jdw_kerja_m_id`
                                    LEFT JOIN jam_kerja ON jam_kerja.`jk_id` = jdw_kerja_d.`jk_id`
                                    WHERE jdw_kerja_m.`jdw_kerja_m_kode`='" . $jadwalkerja['kodejk'] . "' AND jdw_kerja_d.`jdw_kerja_d_idx`='" . $n . "'")->getRowArray();
                        ...
                        if ($timeawal <= $datenow) {
                            ...
                            if ($cekinmin[0]['cekin'] == NULL || $cekoutmax[0]['cout'] == NULL) {
                                ...
                                if (isNull($datajam)) {
                                    ...
                                    $cmasuk = $datajam['smasuk'];
                                    $ckeluar = $datajam['skeluar'];
                                }
                                // }
                                $clor = "white";
