...
class Laporan extends BaseController
{
    ...
    public function Tabelabsenlaporanbulanan()
    {
        if ($this->request->isAJAX()) {
            ...
            $getbulan = date('m', strtotime(date($bulan2)));
            $gettahun = date('Y', strtotime(date($bulan2)));

            $client = \Config\Services::curlrequest();

            $getapi = $client->request('GET', "https://apiliburnasional.vercel.app/api?bulan=$getbulan&tahun=$gettahun");

            $datalibur = json_decode($getapi->getBody());
