...
class Mutasiproduksi extends BaseController
{
    ...
    public function Editselected()
    {
        if ($this->request->isAJAX()) {
            if ($this->request->getMethod() == 'post') {
                ...
                $sec = $this->request->getVar('seq');
                $tgl = $this->request->getPost('tglselected');

                $jumlahselect = count($sec);

                for ($i = 0; $i < $jumlahselect; $i++) {

                    $idsec = $sec[$i];

                    $datachecklist = $this->mutasiproduksi->Kodechecklist($idsec);

                    $tgls = date('d', strtotime($tgl));
                    $bln = date('m', strtotime($tgl));
                    $thn = date('y', strtotime($tgl));
                    $nosurat = $tgls . '/' . $datachecklist['kodemutasi'] . '/' . $bln . '/' . $thn;

                    $bulantahun =  '/' . $bln . '/' . $thn;
                    $datainput = [
                        'bulantahun' => $bulantahun,
                        'lokasal' => $datachecklist['lokasi_asal']
                    ];
                    ...
                    if ($cekhdr == 0) {
                        ...
                        if ($cekhdrbon > 0) {
                            ...
                            $tambahmutasihdr = [
                                'kode' => $nosurat,
                                'tgl' => $tgl,
                                'lokasi_asal' => $datachecklist['lokasi_asal'],
                                'lokasi_tujuan' => $datachecklist['lokasi_tujuan'],
                                'analyst_asal' => $datachecklist['analyst_asal'],
                                'analyst_tujuan' => $datachecklist['analyst_tujuan'],
                                'kode2' => $nourut . '/' . $nosurat,
                                'created_at' => date('Y-m-d H:i:s'),
                                'updated_at' => date('Y-m-d H:i:s'),
                            ];
                    ...
                    $dataupdate = $this->mutasiproduksi->where('sec', $idsec)->set($dtl)->update();
                };
    ...
    public function ModalCetak()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'sukses' => view('exim/bjadi/mutasiproduksi/modalcetak', $data)
            ];
    ...
    function Cetak()
    {
        if ($this->request->isAJAX()) {
            ...
            $header = $this->mutasiproduksi->Header($kode2);

            $listitem = $this->mutasiproduksi->Itemreport($kode2);
            ...
            $tampiltabel = [
                'sukses' => view('exim/bjadi/mutasiproduksi/pdf', $data)
            ];
    ...
    public function Datatablesmutasiproduksi()
    {
        ...
        $pilihasal = $this->request->getPost('pilihasal');
        ...
        if ($request->getMethod(true) == "POST") {
            ...
            foreach ($lists as $gs) {
                $no++;
                $row = [];
                $row[] = "<input type='checkbox' name='seq[]' class='form-control checklistseq' value='$gs->sec'>";
                if ($gs->kodemutasi == 'FF/WH') {
                    $row[] = $gs->kode2 . '/' . $gs->hpl;
                } else {
                    $row[] = $gs->kode2;
                }
