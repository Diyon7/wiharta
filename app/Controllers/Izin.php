...
class Izin extends BaseController
{
    ...
    public function Index()
    {
        ...
        if (in_groups('vendor')) {
            ...
            $dataket = $dataketkode['pembagian3_nama'];
        } else {
            $dataket = '%';
        }

        $daftarkaryawanjp3 = $this->pegawaimodel->Daftarkaryawanjp3($dataket);
        $daftarkaryawan = $this->pegawaimodel->Daftarkaryawana();
    ...
    public function Fio()
    {
        ...
        if ($this->request->isAJAX()) {
            ...
            if ($idkar != '' && $tanggal != '' && $shift != '') {
                ...
                $outmin = date('Y-m-d H:i:s', strtotime('+2 hours', strtotime($tanggal . ' ' . $jamdata['jk_bcin'])));
                $outmax = date('Y-m-d H:i:s', strtotime('+15 hours', strtotime($tanggal . ' ' . $jamdata['jk_bcin'])));
    ...
    public function Addizin()
    {
        ...
        if (date("D") == "Mon") {
            if (in_groups('vendor')) {
                $datenow = date("Y-m-d 00:00:00", strtotime("- 9 day"));
                if (htmlspecialchars($this->request->getPost('unit')) == 'SUBKON MSL LAMONGAN QC' || htmlspecialchars($this->request->getPost('unit')) == 'SUBKON MSL LAMONGAN KEB' || htmlspecialchars($this->request->getPost('unit')) == 'SUBKON MSL LAMONGAN P6') {
                    $datenow = date("Y-m-d 00:00:00", strtotime("- 10 day"));
                }
                // $datenow = date("Y-m-d 00:00:00", strtotime("- 90 day"));
            } else {
                $datenow = date("Y-m-d 00:00:00", strtotime("- 90 day"));
            }
        } else if (date("D") == "Tue") {
            if (in_groups('vendor')) {
                $datenow = date("Y-m-d 00:00:00", strtotime("- 8 day"));
                if (htmlspecialchars($this->request->getPost('unit')) == 'SUBKON MSL LAMONGAN QC' || htmlspecialchars($this->request->getPost('unit')) == 'SUBKON MSL LAMONGAN KEB' || htmlspecialchars($this->request->getPost('unit')) == 'SUBKON MSL LAMONGAN P6') {
                    $datenow = date("Y-m-d 00:00:00", strtotime("- 9 day"));
                }
            } else {
            ...
            if (in_groups('vendor')) {
                $datenow = date("Y-m-d 00:00:00", strtotime("- 7 day"));
                if (htmlspecialchars($this->request->getPost('unit')) == 'SUBKON MSL LAMONGAN QC' || htmlspecialchars($this->request->getPost('unit')) == 'SUBKON MSL LAMONGAN KEB' || htmlspecialchars($this->request->getPost('unit')) == 'SUBKON MSL LAMONGAN P6') {
                    $datenow = date("Y-m-d 00:00:00", strtotime("- 8 day"));
                }
            } else {
            ...
            if ($this->request->getMethod() == 'post') {
                ...
                if ($datenow <= date('Y-m-d', strtotime($timein)) || $datenow <= date('Y-m-d', strtotime($timeout))) {
                    ...
                    if ($this->validate($rules)) {
                        ...
                        $shift = htmlspecialchars($this->request->getPost('shift'));
    ...
    public function Formizin()
    {
        ...
        if ($request->getMethod(true) == "POST") {
            ...
            foreach ($lists as $izin) {
                $masuk = new DateTime($izin->fin);
                $pulang = new DateTime($izin->fout);
                $jk = $pulang->diff($masuk);
                $no++;
                $row = [];
                $row[] = $izin->nip;
                $row[] = $izin->nama;
                $row[] = $izin->fin;
                $row[] = $izin->fout;
                $row[] = $jk->d . " HARI " . $jk->h . " JAM " . $jk->h . " MENIT";
                $row[] = $izin->vendor;
                $row[] = $izin->izin;
                $row[] = $izin->createdat;
    ...
    public function Formpizin()
    {
        ...
        if ($request->getMethod(true) == "POST") {
            ...
            foreach ($lists as $izin) {
                $masuk = new DateTime($izin->fin);
                $pulang = new DateTime($izin->fout);
                $dateDiff = intval((strtotime($izin->fout) - strtotime($izin->fin)) / 60);
                ...
                $row[] = $izin->fin;
                $row[] = $izin->fout;
                $row[] = $hours . " JAM " . $minutes . " MENIT";
                // $row[] = $jk->d . " HARI " . $jk->h . " JAM " . $jk->i . " MENIT";
                $row[] = $izin->vendor;
                $row[] = $izin->izin;
                $row[] = $izin->createdat;
    ...
    public function Formphizin()
    {
        $request = Services::request();
        $tblizin = new Tblizin_model($request);
        if ($request->getMethod(true) == "POST") {
            $lists = $tblizin->get_datatablesizinhb();
            $data = [];
            $no = $request->getPost("start");
            foreach ($lists as $izin) {
                $no++;
